...
SPIM_output_full = function(test0, cyear, cmonth, cday, ymd_from, ymd_to)
{
    ...
    summ = test[, .(death_o = sum(death_o)), by = .(run, t, population, age_group)]
    summ3 = test[, .(icu_p = sum(icu_p)), by = .(run, t, population, age_group)]
    summ3b = test[, .(icu_i = sum(icu_i)), by = .(run, t, population, age_group)]
    summ4 = test[, .(bed_p = sum(pmax(0, hosp_p - hosp_undetected_p))), by = .(run, t, population, age_group)]
    summ34 = test[, .(admissions = sum(hosp_undetected_o)), by = .(run, t, population, age_group)]
    ...
    w = rbind(
        ...
        SPIM_output_1var(summ3b, "icu_i", "icu_inc", cyear, cmonth, cday, ymd_from, ymd_to),
